...
app.get('/search', async (req, res) => {
    ...
    try {
        ...
        if (response.rowCount == 0){
            res.json({status: "campground is not in database"});
        } else {
...
app.get('/fit', async (req, res) => {
    ...
    console.log(`Fit ${searchTerm}`);
    try {
        const template = "SELECT * FROM campgrounds WHERE maxlength > $1";
        const response = await pool.query(template, [searchTerm]);
        if (response.rowCount == 0){
            res.json({status: "no campground to facilitate"});
        } else {
            const results = response.rows.map((row) => { return {
                campground: row.name,
                location: row.location,
...
app.get('/elevation', async (req, res) => {
    ...
    if (req.query.direction == 'lower'){
        query = "SELECT * FROM campgrounds WHERE elevation < $1";
    } else {
        query = "SELECT * FROM campgrounds WHERE elevation > $1";
    }
    try {
...
app.post("/addCampground", async (req, res) => {
    ...
    try {
        ...
        const check = await pool.query(template1, [name, location]);
        if (check.rowCount != 0){
            ...
            const response = await pool.query(template2, [name, location, maxlength, elevation, sites, pad]);
            res.json({status: "added"});
        }
    } catch (err){
        // whoops
